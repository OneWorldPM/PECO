                                    ...
                                    <tbody>
                                        ...
                                        if (isset($sessions) && !empty($sessions)) {
                                            foreach ($sessions as $val) {
                                                ?>
                                                <tr style="background-color: <?=($val->is_active==1)?'#d3ffd5':'#ffcece'?>;">
													  ...
													  <a href="<?= base_url() ?>admin/sessions/view_session/<?= $val->sessions_id ?>" class="btn btn-info btn-sm" style="margin-bottom: 5px;">View Session</a>
                                                        ...
                                                        <td>
                                                            <?php if($val->is_active == 1): ?>
                                                                <button class="disable-session-btn btn btn-sm btn-warning margin-top-5" session-id="<?= $val->sessions_id ?>"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> Disable</button>
                                                            <?php else: ?>
                                                                <button class="activate-session-btn btn btn-sm btn-success margin-top-5" session-id="<?= $val->sessions_id ?>"><i class="fa fa-check-circle" aria-hidden="true"></i> Activate</button>
                                                            <?php endif; ?>
                                                        </td>
                                                </tr>
                                                <?php
...
<?php endif; ?>
        ...
        $("#sessions_table").on('click', '.redirect-attendee', function () {
            socket.emit('redirect-attendee', {appName: $(this).attr('app-name'), sessionId: $(this).attr('session-id')});
        });

        $("#sessions_table").on('click', '.disable-session-btn', function () {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "You are about to disable a session!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    $.get('<?=base_url()?>admin/sessions/disableSession/'+sessionId, function (response) {
                        row.css('background-color', '#ffcece');
                        Swal.fire(
                            'Disabled!',
        ...
        $("#sessions_table").on('click', '.activate-session-btn', function () {

            let sessionId = $(this).attr('session-id');
            let row = $(this).parent().parent();

            $.get('<?=base_url()?>admin/sessions/enableSession/'+sessionId, function (response) {
                row.css('background-color', '#d3ffd5');
                Swal.fire(
                    'Activated!',
